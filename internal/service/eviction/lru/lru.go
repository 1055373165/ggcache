...
func NewLRUCache(maxBytes int64, onEvicted func(string, strategy.Value)) *LRUCache {
	l := &LRUCache{
		maxBytes:  maxBytes,
		root:      list.New(),
...
func (c *LRUCache) RemoveOldest() {
	...
	kvPtr, ok := c.root.Remove(ele).(*strategy.Entry)
	if !ok {
		logger.LogrusObj.Error("error: Item in LRU cache is not of type *strategy.Entry")
		return
	}
...
func (c *LRUCache) Get(key string) (value strategy.Value, updateAt *time.Time, ok bool) {
...
func (c *LRUCache) Put(key string, value strategy.Value) {
	// Update Operation
	if ele, ok := c.cache[key]; ok {
		c.root.MoveToBack(ele)
		kv := ele.Value.(*strategy.Entry)
		kv.Touch()
		// update cache nbytes
...
func (c *LRUCache) CleanUp(ttl time.Duration) {
	...
	for e := c.root.Front(); e != nil; {
		next := e.Next()
		if entry, ok := e.Value.(*strategy.Entry); ok && entry != nil {
