...
支持分布式节点间使用 `HTTP` 或 `gRPC`协议通信、支持高可用的服务注册发现、支持动态节点管理和网络拓扑快速收敛的分布式键值缓存系统。
...
- v4 version, 实现了
  -   整体代码重构
  -   对缓存分段以减小锁粒度从而提升并发性能
  -   增加指标采集实时监控（prometheus、grafana），本地或者容器启动
  -   增加高级缓存淘汰算法 arc
  -   优化测试、CI/CD
...
8. 实现缓存和数据库的一致性（增加消息队列异步处理）（也可以通过缓存淘汰时的回调函数实现） todo（重点）
