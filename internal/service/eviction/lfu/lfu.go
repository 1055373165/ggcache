...
func NewLFUCache(maxBytes int64, onEvicted func(string, strategy.Value)) *lfuCache {
...
func (p *lfuCache) Get(key string) (value strategy.Value, updateAt *time.Time, ok bool) {
...
func (p *lfuCache) Put(key string, value strategy.Value) {
	if e, ok := p.cache[key]; ok {
		...
		e := &lfuEntry{0, strategy.Entry{Key: key, Value: value, UpdateAt: nil}, 0}
		e.referenced()
		heap.Push(p.pq, e)
		p.cache[key] = e
		// update memory usage
