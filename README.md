...
支持分布式节点间使用 `HTTP` 或 `gRPC`协议通信、支持服务注册发现的外部存储中心集群部署、支持动态节点管理和网络拓扑快速收敛的分布式键值缓存系统；
...
> 在动态节点管理实现时，使用 channel 实现了负责哈希环视图重建的 goroutine 和负责监听 endpoint 事件 goroutine 之间的通信。
